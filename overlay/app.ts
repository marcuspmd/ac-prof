...
function speakFeedback(message: string, type: "neutral" | "warning" | "danger" | "success"): void {
  const now = Date.now();
  ...
  else coachPanel.classList.add("coach-neutral");
...
window.onTelemetryUpdate = function(data: TelemetryData): void {
  ...
  if (gear === 0) gearIndicator.innerText = "R";
  else if (gear === 1) gearIndicator.innerText = "N";
  else gearIndicator.innerText = (gear - 1).toString();
